...
class _TextSelMixin:
    ...
    def _get_page_words(self, pn: int) -> list[tuple]:
        ...
        # Reading order: y-band of 5 pt, then left-to-right
        words.sort(key=lambda w: (round(w[0].y0 / 5) * 5, w[0].x0))
    ...
    def _show_text_sel_bar(self, text: str) -> None:
        ...
        if sel_rect is not None:
            popup.left = max(0.0, sel_rect.x0 * scale)
            popup.top  = sel_rect.y1 * scale + 8
        popup.visible = True
        try:
            popup.update()
        except Exception:
