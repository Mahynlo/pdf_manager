...
- **Fuentes adicionales para el texto (FreeText)**: el editor ofrece ahora las **12 variantes** de las tres familias base PDF — Helvetica, Times y Courier (monoespaciada), cada una en **regular, negrita, cursiva y negrita-cursiva** (`FREETEXT_FONTS`). Siguen siendo fuentes base estándar (sin incrustar archivos, con soporte de acentos y ñ).
- **Leyendas de texto (plantillas reutilizables)**: nuevo botón en la barra de anotaciones que abre un **menú de inserción rápida** con tus textos pre-escritos para insertarlos como anotación FreeText con un clic, sin volver a teclearlos ni reconfigurarlos. Cada leyenda guarda su **estilo completo** (fuente, tamaño, color, alineación, borde) además del texto, así se inserta ya formateada y se puede modificar al colocarla. El menú rápido muestra las **más usadas** (ordenadas por nº de inserciones y, como desempate, por uso más reciente). Un gestor permite **crear, editar, buscar y eliminar** leyendas. Persisten entre sesiones en `~/.extraer_pdfs_legends.json` (mismo patrón singleton + JSON que los perfiles de censura).
- **Duplicar anotación**: copia una anotación seleccionada desplazada respecto al original, conservando **estilo, texto (FreeText) y rotación** (`duplicate_annot`). Reutiliza el mismo serializado de deshacer/rehacer (`_snapshot_annot` + `_translate_snapshot` + `_recreate_annot`), por lo que la copia entra en el historial como una acción más.

### Changed
- **Empaquetado más liviano (optimización del instalador, sin cambios de funcionalidad)**: se redujo el tamaño del bundle de Flet con cambios solo de configuración. (1) Se **eliminó `pywin32`** de las dependencias (~17 MB): no se importaba en ningún sitio — el único uso de Win32 (`GetCommandLineW` en `main._win32_cmdline_args`) va por `ctypes` y la impresión es sin pywin32. (2) Se **excluyen del bundle `flet_desktop` (~95 MB) y `flet_web` (~12 MB)**: el primero solo se usa en modo dev (`flet run`) — en la app empaquetada el Python corre embebido en el runner de Flutter (`is_embedded()=True`) y nunca se importa; el segundo solo lo usa el servidor web, no la app desktop. (3) Se activó la **compilación a `.pyc`** del código de la app y de los paquetes (`[tool.flet.compile]`), eliminando los `.py` fuente del bundle, y se **purgan `*.pyi` y suites de `tests/`** (`[tool.flet.cleanup]`). Las extensiones nativas y los datos de modelos OCR no se tocan.

## [0.1.20] - 2026-06-10

### Added
- **Modo oscuro en toda la aplicación**: el interruptor de **Configuración → Modo oscuro** (que cambia `page.theme_mode`) ahora repinta **toda** la UI, no solo Ajustes. Se migraron los colores fijos (hex) a **tokens de tema de Material** (`onSurface`, `onSurfaceVariant`, `surface`, `surfaceVariant`, `outlineVariant`, `outline`, `primary`/`primaryContainer`, `error`) en: Inicio, Seguridad, Combinar PDFs (+ widgets), Extraer (+ strings de log del engine), la **barra de pestañas** y el **visor completo** (`_viewer_defs`, `viewer`, y los mixins de render/OCR/censura/perfiles/impresión/agente). Los fondos pastel con identidad de color pasaron a tintes translúcidos (`ft.Colors.with_opacity`) del acento, y los mensajes de error/éxito a rojo/verde con fondo translúcido. **Se conservan deliberadamente** los colores que NO son "chrome": los colores reales escritos al PDF (anotaciones, paleta y relleno de censura), los overlays dibujados sobre la imagen de la página (selección, tiradores, *ghost*, tinta, cajas OCR), el "papel" blanco de la página y el **modo noche** del visor (independiente del tema de la app).
- **Botón «+» en la barra de pestañas para abrir un PDF en una pestaña nueva**: como en un editor. Tiene comportamiento **híbrido** según el espacio: mientras las pestañas caben sin scroll, el «+» aparece como una mini-pestaña **pegada a la derecha de la última**; cuando hay tantas que la barra hace scroll, se oculta esa variante y aparece un «+» **fijo** al final de la barra, para poder abrir otro PDF sin recorrer toda la fila. Ambas variantes llaman al selector de archivos (misma acción que «Abrir PDF»).

### Changed
- **Barra superior eliminada: marca y acciones integradas en la barra de pestañas**: se quitó el *navbar* azul marino propio. Ahora el nombre/ícono de la app («Extraer PDFs») quedan **fijos a la izquierda de las pestañas**, junto a un botón de **menú «☰»** desplegable que agrupa las acciones que antes estaban en la barra superior (Abrir PDF, Extraer texto, Combinar PDFs, Seguridad y Configuración). `DocumentManagerUI` expone `set_leading()` para inyectar ese widget desde `main`, donde viven los callbacks. Resultado: una sola barra, más limpia, y todo se maneja desde ahí.
- **Tab de Seguridad responsivo y con scroll**: el contenido de la pestaña de Seguridad (Desbloquear/Proteger) se adapta al tamaño de la ventana — las dos columnas se **apilan** en pantallas estrechas (`ft.ResponsiveRow`) y el panel hace **scroll vertical** cuando la ventana es baja, de modo que la sección de **permisos** ya no queda cortada e inalcanzable.
- **Agente IA oculto temporalmente (en pulido)**: se añadió un *feature flag* `AGENT_ENABLED` (en `_viewer_defs.py`, hoy `False`) que oculta la **pestaña «Agente IA»** del panel lateral y el **botón del agente** en la barra de herramientas, de modo que no hay forma de invocarlo desde la UI mientras se perfecciona. El código del agente queda **intacto**: reactivarlo es poner el flag en `True`. Motivo principal: el agente envía contenido del PDF a un LLM externo (Gemini/OpenAI), algo a revisar con cuidado en una app que también censura datos sensibles. (Ya era opt-in —requería API key— y no era la vista por defecto.)
- **Censura — búsqueda siempre por palabra completa**: un término coincide **solo como palabra entera** — buscar «la» ya **no** coincide dentro de «tabla» o «regla». Es el comportamiento esperable al censurar términos concretos y elimina de raíz la explosión de coincidencias con palabras comunes. PyMuPDF no tiene un flag nativo de palabra completa (`search_for` siempre hace matching de subcadena), así que se implementa matcheando contra los **tokens** de `get_text("words")` por igualdad (con paridad en la búsqueda OCR vía límites `\b`). Se siguen pudiendo buscar **frases** (matchea palabras consecutivas en orden, tolerando la puntuación). No es configurable: la búsqueda por subcadena se eliminó. Verificado: en «la tabla de la casa regla», la subcadena daba 4 coincidencias y la palabra completa da 2.
- **Censura — búsqueda insensible a acentos**: los términos se comparan plegando tildes y diéresis a la vocal base, así que «COMITÉ» coincide con «COMITE» y «Pública» con «Publica» (es habitual escribirlos sin acento). La **ñ/Ñ se preserva** a propósito: es una letra propia del español, no una «n con tilde», así que «año» ≠ «ano». Aplica tanto al texto nativo del PDF como a la búsqueda sobre OCR (tabla de plegado 1:1, así las posiciones de caracteres en OCR no se desfasan).

### Fixed
- **«Abrir con» mostraba el ícono genérico de Windows en vez del de la app**: el handler de «Abrir con» es `launcher.exe` (puente IPC), que se compilaba desde `launcher.c` **sin recurso de ícono**, así que Windows no tenía ícono embebido que mostrar y caía en el genérico. Se añadió `launcher/launcher.rc` (`1 ICON "app.ico"`) y un `launcher/app.ico` multi-resolución (16–256 px) generado desde `src/assets/icon.png`; `build.ps1` y el workflow de CI ahora compilan el recurso con **windres** y lo enlazan al `launcher.exe`. Si `windres` no estuviera disponible, el launcher se compila sin ícono (no se aborta el build por algo cosmético). Nota: tras instalar la nueva versión, Windows puede tardar en refrescar su caché de íconos.
- **Censura — congelamiento al buscar palabras muy comunes («de», «la», «las»…)**: la búsqueda se trababa por dos motivos, ya corregidos. (1) `_find_term_matches` llamaba a `page.get_textbox(r)` (código nativo) **por cada coincidencia** para una etiqueta que **nunca se usa** (ni `_apply_redaction` ni la vista previa la leen) — con una palabra común eran miles de llamadas desperdiciadas; ahora se guarda el término directamente. (2) Se añadió un **cortacircuitos** (`_REDACT_MAX_MATCHES = 1000`): al alcanzar el tope se corta el barrido y, en la ruta interactiva, se avisa al usuario que refine el término (las rutas de lote cortan en silencio). Combinado con la búsqueda por palabra completa (siempre activa), buscar conectores comunes deja de saturar el hilo de UI.
